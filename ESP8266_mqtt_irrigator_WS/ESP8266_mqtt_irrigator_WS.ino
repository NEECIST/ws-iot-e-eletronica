...
#define EIOTCLOUD_USERNAME "xxxx"
#define EIOTCLOUD_PASSWORD "xxxx"
...
#define PIN_PUMP         D0  // LED do nodeMCU
...
#define PARAM_HUMIDITY_TRESHOLD   "Sensor.Parameter1" //Threshold de humidade
#define PARAM_MANUAL_AUTO_MODE    "Sensor.Parameter2" //manual (0), auto (1)
#define PARAM_PUMP_ON             "Sensor.Parameter3" //Estado da bomba
...
int soilHumidityThreshold = 50; // Limiar da humidade do solo
...
void setup() {
  ...
  digitalWrite(PIN_PUMP, HIGH); // enviamos HIGH para parar a bomba
  ...
  if (IsTimeout()) // Executa a cada segundo
  {
    ...
    switch (state)
    {
      case s_idle:
        ...
        //Só regamos se o PAUSE_TIME(300s) já tiver passado
      ...
      case s_irrigation_start:
        /***************INCOMPLETE*******************/
        //completar- dar reset ao contador, ligar a bomba, enviar a informação para o server, alterar o estado
...
boolean IsTimeout()
{
  ...
  if (startTime <= now)
  {
    if ( (unsigned long)(now - startTime )  < MS_IN_SEC ) //ainda nao passou 1 seg
      return false;
  }
  else
  {
    if ( (unsigned long)(startTime - now) < MS_IN_SEC ) //para evitar bugs
      return false;
  }
...
void subscribe()
{
  ...
  {
    /**********INCOMPLETE**********/
    //subscrever aos tópicos em que queremos receber informações
...
void myDataCb(String& topic, String& data) {
  ...
  if (topic == String("/" + String(storage.moduleId) + "/" + PARAM_HUMIDITY_TRESHOLD)) // Executa se a mensagem que receber for relativa a uma variação da humidade limiar do solo
  {
    ...
    Serial.print("Received instruction to change humidity threshold to:");
  ...
  else if (topic == String("/" + String(storage.moduleId) + "/" + PARAM_PUMP_ON)) // Executa se a mensagem que receber for relativa a ligar ou desligar a bomba de aguá
  {
    if (data == String("1")){
      state = s_irrigation_start;
      Serial.println("Received instruction to start irrigation");

    }else{
      state = s_irrigation_stop;
      Serial.println("Received instruction to stop irrigation");
    }
